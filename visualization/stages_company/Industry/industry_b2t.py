...
def update_patent_counts(company_csv, patent_csv, output_csv,
    ...
    df_companies = pd.read_excel(company_csv)
    df_patents = pd.read_excel(patent_csv)
    ...
    df_companies['Normalized Name'] = df_companies['coname'].apply(lambda x: normalize_terms(x.lower(), term_map))
        ...
        for assignee_normalized in assignees:
            if assignee_normalized:
                ...
                if match:
                    ...
                    if score >= threshold:
                        matched_company = df_companies[df_companies['Normalized Name'] == best_match]['coname'].iloc[0]
                        matched_idx = df_companies[df_companies['coname'] == matched_company].index
...
update_patent_counts('/Users/asa/PycharmProjects/stages_company/Industry/company_industry.xlsx', '/Users/asa/PycharmProjects/stages_company/Industry/patent.xlsx', 'b2t1.csv')
